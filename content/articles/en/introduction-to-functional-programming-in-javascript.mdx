...
```js
// Impureconst minimum = 21;const checkAge = age => age >= minimum;// Pureconst checkAge = (age) => {  const minimum = 21;  return age >= minimum;};
```
...
```js
// Mutableconst person = {  name: 'Vitor',  age: 28,};person.age = 30; // OK// Immutableconst person = Object.freeze({  name: 'Vitor',  age: 28,});person.age = 30; // TypeError: Cannot assign to read only property 'age' of object '#<Object>'
```
...
```js
// Assigning functions to variablesconst add = (a, b) => a + b;// Passing functions as arguments to other functionsconst add = (a, b) => a + b;const subtract = (a, b) => a - b;const fcFn = (func) => func(2, 3);fcFn(add); // 5fcFn(subtract); // -1
```
...
```js
// Abstract or isolate actions, effects, or operationsconst withCount = fn => {  let count = 0;  return (...args) => {    console.log(`Call count: ${++count}`);    return fn(...args);  };};const add = (x, y) => x + y;const countedAdd = withCount(add);countedAdd(1, 2); // Call count: 1countedAdd(2, 2); // Call count: 2
```
...
curriedMultiply(3)(4); // 12const multiplyBy5 = curriedMultiply(5);multiplyBy5(4); // 20
```
...
```js
const getUserDisplayName = user => `${user.firstName} ${user.lastName}`const users = [  {firstName: 'Jane', lastName: 'Doe'},  {firstName: 'John', lastName: 'Doe'}]users.map(getUserDisplayName) // ["Jane Doe", "John Doe"]
```
...
By Vitor Britto on May 29, 2023.
