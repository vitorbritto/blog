...
First, we need to care about where to save the data when the user is offline, for that I recommend using RealmDB or [WatermelonDB](https://github.com/Nozbe/WatermelonDB). Even so, if you want, you can use AsyncStorage without problems, the only limitation is that in this case you will not be able to perform any type of filter or query on the data recorded offline.
Now that we have where to record the data, we need to find out if the user is online or not. React Native exposes the [NetInfo](https://github.com/react-native-netinfo/react-native-netinfo) native API that gives us this information.

## Using WatermelonDB for complex data

If your React Native app is simple and you use remote services to store your data in a database and Redux throughout the app, then [react-native-offline](https://github.com/rgommezz/react-native-offline) and [redux-persist](https://github.com/rt2zz/redux-persist) will work for you.
