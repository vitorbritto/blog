...
```graf graf--pre graf-after--li graf--preV2
input CreateUserInput {  username: String!  password: String!  email: String!}
```
...
```graf graf--pre graf-after--li graf--preV2
interface Entity {  id: ID!  name: String!}type User implements Entity {  id: ID!  name: String!  email: String!}type Product implements Entity {  id: ID!  name: String!  price: Float!}
```
...
```js
{  "data": {    "user": {      "__typename": "CommonUser",      "id": 1,      "name": "John Doe"    }  }}
```
...
```js
fragment UserInfo on User {  id  name}{  user(id: 1) {    ...UserInfo  }  anotherUser: user(id: 2) {    ...UserInfo  }}
```
...
```js
{  user(id: 1) {    id    name    posts @include(if: $includePosts) {      title      content    }  }}
```
...
```js
type Query {  listUsers(): [User!]!}type Mutation {  createUser(name: String!, age: Int!): User!}type Subscription {  userCreated: User}
```
...
```js
query {  user(id: "1") {    id    name    email    posts {      id      title      content      attachments {        id        filename        url      }    }    role  }}gr
```
...
```js
mutation {  createUser(input: { name: "John Doe", email: "john@doe.com" }) {    id    name    email  }}
```
...
```js
const resolvers = {  Query: {    user: (parent, args, context, info) => {      // Access context data      const { userId } = context;          // Fetch and return user data based on userId      return fetchUserData(userId);    }  }};
```
...
By Vitor Britto on November 24, 2023.
