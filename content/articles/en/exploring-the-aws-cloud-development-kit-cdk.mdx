...
```js
import * as cdk from 'aws-cdk-lib';import { Construct } from 'constructs';import * as s3 from 'aws-cdk-lib/aws-s3';export class MyCdkAppStack extends cdk.Stack {  constructor(scope: Construct, id: string, props?: cdk.StackProps) {    super(scope, id, props);    // Defining an S3 bucket    new s3.Bucket(this, 'MyFirstBucket', {      versioned: true, // Enable versioning      removalPolicy: cdk.RemovalPolicy.DESTROY, // Specify what happens to the bucket when the stack is deleted      autoDeleteObjects: true, // Automatically delete objects in the bucket when the stack is deleted    });  }}
```
...
```js
import * as cdk from 'aws-cdk-lib';import { Construct } from 'constructs';import * as s3 from 'aws-cdk-lib/aws-s3';export class MyCdkAppStack extends cdk.Stack {  constructor(scope: Construct, id: string, props?: cdk.StackProps) {    super(scope, id, props);        // Defining an S3 bucket    new s3.Bucket(this, 'MyFirstBucket', {      versioned: true,      removalPolicy: cdk.RemovalPolicy.DESTROY,      autoDeleteObjects: true,    });  }}
```
