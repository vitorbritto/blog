...
excerpt: The CI/CD automation on GitHub
---

An introduction to Github ActionsAn introduction to Github Actions
...
```js
name: CIon: [push, pull_request]jobs:  build:    runs-on: ubuntu-latest    steps:    - name: Checkout code      uses: actions/checkout@v2    - name: Set up Node.js      uses: actions/setup-node@v2      with:        node-version: '18'    - name: Install dependencies      run: npm install    - name: Run tests      run: npm test
```
...
By Vitor Britto on June 22, 2024.
